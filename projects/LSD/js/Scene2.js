...
class Scene2 {
  ...
  zipperTeeth(){
    ...
    rectMode(CENTER)
    if (scene === "scene2"){
      for(let i = zip.teethY; i < height; i += zip.teethY){ // Prints 2 rows of rectangles going down the middle of the screen
        if (zip.clicked === true && mouseY > i){ // The if statement prevents the teeth which have been opened by the zip from being displayed
          console.log("cheese")
        }
        else {
  ...
  openZip(){
    if (zip.clicked === true && scene === "scene2"){
      ...
      if (mouseY > height - 100){
        scene = "scene3" // Switches scenes when the zip has been opened enough
      }
    }
  }
