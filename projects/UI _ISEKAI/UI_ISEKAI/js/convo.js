  ...
  (function ($) {
    $.fn.convo = function (options) {
      ...
      var settings = $.extend(
        {
          ...
          getPath: function (pathName) {
            for (var i = 0, len = settings.data.length; i < len; i++) {
              if (settings.data[i].path === pathName) return settings.data[i]; // Return as soon as the object is found
            }
      ...
      $("body").on("keydown", ".cui_option_input", function (e) {
        if (
          e.which !== 8 &&
          settings.checkCuiInputLength(settings.inputCharLimit) === true
        ) {
          e.preventDefault();
          return;
        }
